...
const SubmitPaperPage: React.FC = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      setFormData(prevState => ({ ...prevState, fullPaperFile: e.target.files![0] }));
    }
  };
    ...
    <div className="max-w-2xl mx-auto bg-slate-800/50 backdrop-blur-sm p-8 rounded-lg shadow-lg border border-slate-700/50">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="authorName" className={labelStyles}>Họ và tên Tác giả <span className="text-red-500">*</span></label>
              <input type="text" name="authorName" id="authorName" value={formData.authorName} onChange={handleChange} required className={inputStyles} />
            </div>
             <div>
              <label htmlFor="organization" className={labelStyles}>Đơn vị công tác <span className="text-red-500">*</span></label>
              <input type="text" name="organization" id="organization" value={formData.organization} onChange={handleChange} required className={inputStyles} />
            </div>
            <div>
            ...
            <div>
              <label htmlFor="phone" className={labelStyles}>Số điện thoại</label>
        ...
        <div>
            <label htmlFor="paperTitle" className={labelStyles}>Tên bài báo <span className="text-red-500">*</span></label>
            <input type="text" name="paperTitle" id="paperTitle" value={formData.paperTitle} onChange={handleChange} required className={inputStyles} />
        </div>
        <div>
            <label htmlFor="topic" className={labelStyles}>Chủ đề <span className="text-red-500">*</span></label>
            <select name="topic" id="topic" value={formData.topic} onChange={handleChange} className={inputStyles}>
